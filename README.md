...
3. Pick the repo. When asked for build settings, use:
   ...
   >
   > ⚠️ **Do not include a `_redirects` file either** (unless you actually need URL rewrites). Pages treats `/index.html` as a pretty-URL alias for `/`, so a catch-all `/* → /index.html` rule creates an infinite redirect loop and fails the deploy.
